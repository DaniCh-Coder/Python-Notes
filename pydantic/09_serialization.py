...
def main() -> None:
    ...
    if user:
        print(
            "1. serializador model_dump retorna un dict:",
            user.model_dump(),
            sep="\n",
            end="\n\n",
        )
        print(
            "2. serializador model_dump_json retorna JSON string:",
            user.model_dump_json(),
            sep="\n",
            end="\n\n",
        )
        print(
            "3. serializador que retorna json string, excluyendo el rol:",
            user.model_dump(exclude=["role"], mode="json"),
            sep="\n",
            end="\n\n",
        )
        print("4. serializador que codifica los valores en un diccionario", dict(user), sep="\n")
